...
// changelog: added classes, added final boss, leveling up scales the enemies, 
...
public class encounters
{
    ...
    {
        Console.Clear();
        Console.WriteLine("As you walk into the next room you see a giant goblin. Taller than you!");
        Console.WriteLine("It turns to you.");
        Console.ReadKey();
        combat (false, "Giant", 6, 20);
    ...
    public static void combat(bool random, string name, int power, int health)
    {
        ...
        while (h > 0)
        {
            ...
            else if (input.ToLower() == "d" || input.ToLower() == "defend")
            {
                ...
                if (damage < 0)
                    damage = 0;
                int attack = rand.Next(0, program.currentPlayer.weaponValue) + rand.Next(1, 4)/2 + ((program.currentPlayer.currentClass == Player.PlayerClass.Warrior)?1:0);
                Console.WriteLine("You lose " + damage + " health and deal " +attack+ " damage.");
